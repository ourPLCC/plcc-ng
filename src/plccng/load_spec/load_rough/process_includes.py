...
def process_includes(rough):
    return IncludeProcessor().process(rough)
...
class IncludeProcessor():
    ...
    def _process_part(self, part):
        if isinstance(part, Include):
            yield from self._process_include(part)
        else:
            yield part
