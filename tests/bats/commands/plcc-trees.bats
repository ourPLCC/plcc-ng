...
@test "plcc-trees --help exits 0" {
    run plcc-trees --help
    [ "$status" -eq 0 ]
}

@test "plcc-trees requires --ll1" {
    run bash -c "echo '{}' | plcc-trees"
    [ "$status" -ne 0 ]
}

@test "plcc-trees dispatches to plcc-parser-table by default" {
    run bash -c "echo '42' | plcc-tokens '${SPEC_JSON}' | plcc-trees --ll1='${LL1_JSON}'"
    [ "$status" -eq 0 ]
    echo "$output" | check-jsonschema --schemafile "${TREE_SCHEMA}" -
}

@test "plcc-trees errors on missing parser plugin" {
    run bash -c "echo '42' | plcc-tokens '${SPEC_JSON}' | plcc-trees --ll1='${LL1_JSON}' --parser=nonexistent 2>&1"
    [ "$status" -ne 0 ]
    [[ "$output" == *"not found"* ]]
}

@test "plcc-trees passes error records through" {
    ERROR='{"kind":"error","stage":"plcc-tokens","source":{"file":null,"line":1,"column":1}}'
    run bash -c "echo '${ERROR}' | plcc-trees --ll1='${LL1_JSON}'"
    [ "$status" -eq 0 ]
    echo "$output" | python3 -c "import json,sys; r=json.load(sys.stdin); assert 'error' in json.dumps(r)"
}

@test "plcc-trees accepts -v without error" {
    run bash -c "echo '42' | plcc-tokens '${SPEC_JSON}' | plcc-trees --ll1='${LL1_JSON}' -v"
    [ "$status" -eq 0 ]
}
