...
@test "plcc-make does not rebuild when spec is unchanged" {
    ...
    rm plcc-ng/ll1.json          # manually remove artifact
    plcc-make --through=parse  # spec unchanged → fast path
    [ ! -f plcc-ng/ll1.json ]    # not rebuilt confirms fast path taken
}

@test "plcc-make --through=all fast-paths when all-level sentinel present" {
    cp "${FIXTURES}/trivial-python.plcc" spec.plcc
    plcc-make
    rm plcc-ng/ll1.json          # manually corrupt plcc-ng/
...
@test "plcc-make rebuilds when spec changes" {
    ...
    first_spec=$(cat plcc-ng/spec.json)
...
@test "plcc-make --through=all then --through=scan fast-paths" {
    ...
    [ ! -f plcc-ng/ll1.json ]   # not rebuilt confirms fast path
}
...
@test "plcc-make writes plcc-ng/.spec after successful build" {
    cp "${FIXTURES}/trivial.plcc" spec.plcc
    plcc-make --through=scan
    [ -f "plcc-ng/.spec" ]
    [[ "$(cat plcc-ng/.spec)" == "spec.plcc" ]]
}

@test "plcc-make with --spec writes that path to plcc-ng/.spec" {
    run plcc-make --through=scan "--spec=${FIXTURES}/trivial.plcc"
    [ "$status" -eq 0 ]
    [[ "$(cat plcc-ng/.spec)" == "${FIXTURES}/trivial.plcc" ]]
}
...
@test "plcc-make with --spec differing from stored wipes build" {
    ...
    # plcc-ng/ was wiped and rebuilt from other.plcc
    [[ "$(cat plcc-ng/.spec)" == "other.plcc" ]]
}

@test "plcc-make stored spec missing gives error to stderr with hint" {
    mkdir -p plcc-ng
    echo "ghost.plcc" > plcc-ng/.spec
    run --separate-stderr plcc-make
    [ "$status" -ne 0 ]
    [[ "$stderr" == *"spec file not found"* ]]
    [[ "$stderr" == *"ghost.plcc"* ]]
...
@test "plcc-make no plcc-ng/.spec no --spec falls back to spec.plcc" {
    run --separate-stderr plcc-make
    [ "$status" -ne 0 ]
    [[ "$stderr" == *"spec.plcc"* ]]
}

@test "plcc-make writes plcc-ng/.spec even when plcc-spec fails" {
    printf 'token BAD @@@\n' > bad.plcc
    run plcc-make --spec=bad.plcc
    [ "$status" -ne 0 ]
    [ -f "plcc-ng/.spec" ]
    [[ "$(cat plcc-ng/.spec)" == "bad.plcc" ]]
}
