...
Commits follow conventional-commit style (`feat(scope): …`, `fix(scope): …`, `docs(scope): …`, `test(scope): …`, `refactor(scope): …`, `build(scope): …`, `ci: …`, `chore: …`). Match the scope names already in use in the git log. Pure-internal commits (issue files, plans, specs — anything that does not change content under `docs/` or `dev-docs/`) should append `[skip ci]` to the subject line to avoid triggering the test and docs-deployment workflows unnecessarily. Do **not** use `[skip ci]` on commits that modify `docs/`, `dev-docs/`, `mkdocs.yml`, `mkdocs-dev.yml`, or `CHANGELOG.md` — those changes need the docs deployment workflow to run so the site stays current.
